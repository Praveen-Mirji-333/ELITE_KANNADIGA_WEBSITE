...
export default function Page() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#050505] text-[#f5f5f5]">
      ...
      {/* Hero Section */}
      <section className="hero relative flex min-h-[780px] items-center pt-20">
        <div className="hero-grid absolute inset-0" />
        <div className="hero-glow absolute right-[5%] top-[18%] h-[520px] w-[520px] rounded-full bg-red-950/40 blur-[120px]" />
        <div className="relative z-10 mx-auto grid w-full max-w-[1440px] items-center gap-10 px-5 py-20 lg:grid-cols-[1.05fr_.95fr] lg:px-10">
          <div className="max-w-2xl">
            <p className="eyebrow mb-5 flex items-center gap-3">
              <span className="h-px w-8 bg-red-600" /> THE OFFICIAL GAMING COMMUNITY
            </p>
            <h1 className="font-display text-[clamp(4rem,10vw,8.5rem)] font-black uppercase leading-[.78] tracking-[-.08em] text-white">
            ...
            <div className="mt-9 flex flex-wrap gap-3">
              <Link
                href="/tournaments"
                className="cut-button bg-red-600 px-6 py-4 text-xs font-black tracking-[.18em] text-white transition hover:bg-red-500"
              >
                ENTER TOURNAMENTS <ArrowUpRight className="ml-3 inline" size={16} />
              </Link>
              {SHOW_LIVE_SECTION && (
                <Link
                  href="/live"
                  className="cut-button border border-white/20 bg-white/5 px-6 py-4 text-xs font-black tracking-[.18em] text-white transition hover:border-red-600"
                >
            ...
            {homeCards.length > 0 && (
              <div className="mt-14 grid max-w-xl grid-cols-2 gap-y-7 border-t border-white/10 pt-7 sm:grid-cols-3">
                {homeCards.map((c) => (
                  <div key={c.id}>
                    <p className="font-display text-2xl font-black text-white">
                      {c.stat_number || c.title}
                    </p>
                    <p className="mt-1 text-[9px] font-bold tracking-[.16em] text-white/35">
          ...
          <div className="relative flex justify-center lg:justify-end">
            ...
            <img
              ...
              className="hero-logo relative z-10 w-[min(78vw,580px)] object-contain"
            />
            <div className="absolute bottom-10 left-1/2 z-20 -translate-x-1/2 whitespace-nowrap border border-red-700/60 bg-black/80 px-4 py-2 text-[10px] font-bold tracking-[.25em] text-red-400 backdrop-blur">
      ...
      {/* Tournaments Preview */}
      <section className="border-y border-white/5 bg-[#0b0b0d] px-5 py-24 lg:px-10">
          ...
          {tournamentsList.length === 0 ? (
            ...
            <div className="grid gap-5 xl:grid-cols-3">
              {tournamentsList.map((t) => (
                <article key={t.id || t.title} className="tournament-card red-edge">
                  <div className="mb-7 flex items-start justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-[10px] font-bold tracking-[.2em] text-white/35">
                        {t.game_mode}
                      </p>
                    </div>
                    <Crosshair className="text-red-600" size={25} />
                  </div>
                  <div className="mb-7 grid grid-cols-2 gap-5 border-y border-white/10 py-5">
                    <div>
                  ...
                  <div className="flex items-end justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-[10px] font-bold tracking-widest text-white/40">
                        {t.map || 'BERMUDA'} / {t.game_mode}
                      </p>
                    </div>
                    <Link
                      href="/tournaments"
                      className="border border-red-700 px-3 py-2 text-[10px] font-black tracking-widest text-red-500 transition hover:bg-red-600 hover:text-white"
                    >
